...
import { Logger, TOKEN_PROGRAM_ID, MARKET_STATE_LAYOUT_V3, Liquidity, TokenAmount, simulateTransaction, Token } from "raydium-sdk-opt";
...
async function start() {
    ...
    const baseMintAmount = (0.01) * Number(Info.addLP) * Number(Info.tokenSupply) * 10 ** tokenInfo.decimals;
    const quoteMintAmount = Number(Info.addSol) * 10 ** 9;
    ...
    ammCreatePool({
        ...
        const txMainSwaps: any = await createWalletSwaps(Info.swapWallets,poolKeys, baseToken, blockhash)
...
const createWalletSwaps = async (swapWallets:any,poolKeys: any, baseToken: Token, blockhash: string) => {
    ...
    for (var item of swapWallets) {
        ...
        const outputTokenAmount = new TokenAmount(baseToken, 1, false);
        const inTokenAmount = new TokenAmount(DEFAULT_TOKEN.SOL, item.tokenSwapAmount);
